...
int resizeNode(uint64_t idxNode, size_t newSize) {
    ...
    int diff = newSize - node->fileSize;
    ...
    if(diff > 0) {
        ...
        if(newBlocks) {
            ...
            for(int i = 0; currentBlock != node->numBlocks; i++) {
        ...
        for(int i = node->numBlocks; i > numBlocks; i--) {
...
static int my_unlink(const char *path) {
    int filePos, iNodeNum, numBlocks, iBlock;

    fprintf(stderr, "--->>> my_unlink: path %s\n", path);

    if((filePos = findFileByName(&myFileSystem, (char *) path + 1)) < 0)
        return -ENOENT;
    
    iNodeNum = myFileSystem.directory.files[filePos].nodeIdx;
    numBlocks = myFileSystem.nodes[iNodeNum]->numBlocks;

    // We update the bitmap
    for(int i = 0; i < numBlocks; i++) {
        iBlock = myFileSystem.nodes[iNodeNum]->blocks[i];
        myFileSystem.bitMap[iBlock] = 0;
    }

    // We update i-node
    myFileSystem.nodes[iNodeNum]->fileSize = 0;
    myFileSystem.nodes[iNodeNum]->numBlocks = 0;
    myFileSystem.nodes[iNodeNum]->freeNode = true;

    // We update the Directory
    myFileSystem.directory.numFiles--;
    myFileSystem.directory.files[iNodeNum].freeFile = true;

    // We update the Superblock
    myFileSystem.superBlock.numOfFreeBlocks = myQuota(&myFileSystem);

    // After all changes in memory, we update it in the virtual disk
    updateBitmap(&myFileSystem);
    updateNode(&myFileSystem, iNodeNum, myFileSystem.nodes[iNodeNum]);
    updateDirectory(&myFileSystem);
    updateSuperBlock(&myFileSystem);
